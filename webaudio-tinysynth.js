...
class Drawer {
	...
	update() {
		...
		if (this.synth.hasSong()) {
			...
			ctx.fillStyle = "#fff"
			ctx.fillText(this.toTime(this.synth.getPlayTick()), 38, 14);
			ctx.fillText(this.toTime(this.synth.getMaxTick()), 38, 28);
			ctx.strokeStyle = "#000";
			ctx.beginPath();
			ctx.arc(80 + this.synth.getPlayTick() / this.synth.getMaxTick() * 128, 16, 6, 0, 6.28, 0);
			ctx.fill();
			ctx.stroke();
		}
			...
			if (msg[0] == 0xf0) {
				if (msg[1] == 0x7f && msg[3] == 4) {
					...
					if (msg[4] == 4 && msg.length >= 8)
						this.synth.masterCoarseTuning(msg[6]);
				}
...
/*80*/[{w:"sine",t:0,f:1200,v:0.3,r:0.01,}],
		...
		getPlayTick: ()=>{
			return this.getPlayer().getPlayTick();
		},
		setQuality: (q)=>{
			if (q != undefined)
				this.quality = q;
		...
		masterFineTuning: (value1, value2)=>{
			this.masterTuningF = value1 + value2 * 0x80 - 8192;
		},
		masterCoarseTuning: (value)=>{
			this.masterTuningC = value - 0x40;
		},
		_createWave: (w)=>{
